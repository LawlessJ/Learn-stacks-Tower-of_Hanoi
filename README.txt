The following project and text is provided from codeacademy. Try to recreate this code on your own! Note it is necessary to utilize both the main script.py and stack.py, as the class Stack is defined in that file and imported.

To play the game:
Download all materials, then open your terminal. Navigate your working directory to the directory of all downloaded files, then, in the terminal, type: python3 script.py 
This will execute the code to play the game. Follow onscreen instruction. Each number in stacks represents a disc size (4 is larger than 3, 3 is larger than 2, etc. All of which begin on the left stack. Moving only one disc at a time - and never placing a larger disc onto a smaller disc - move all discs from the left to the right stack.
